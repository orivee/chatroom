...
/* read client config */
void parse_client_config(char * confpath)
{
...
{
    char confpath[256];
    getprogpath(confpath, sizeof(confpath));
    strcat(confpath, "etc/client.conf");
    if( access( confpath, F_OK ) != -1 )
    {
        parse_client_config(confpath);
    }
    else
    {
        // file doesn't exist
